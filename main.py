...
def format_list():
    ...
    for z, u in zones.items():
        display = u if u else "⚪ Vacío"  # Siempre mostrar "Vacío" para zonas sin usuarios
    ...
    mins_left = ROTATION_DURATION_MINUTES - int((datetime.now() - last_rotation_time).seconds / 60) if last_rotation_time else 0
    if mins_left < 0:
        mins_left = 0  # Prevenir números negativos

    return (
        f"⚜️ Estado de la Lista de Zonas ⚜️\n\n"
...
async def reject_private_messages(update: Update):
    if update.effective_chat.type == "private":
        ...
        logger.warning(f"⛔ Mensaje privado rechazado de @{update.effective_user.username}")
        return False
    return True
...
async def check_authorized_chat(update: Update):
    ...
    if current_chat_id != authorized_chat_id:
        ...
        logger.warning(f"⛔ Intento de uso desde chat no autorizado: {current_chat_id} (autorizado: {authorized_chat_id})")
        return False
...
async def cmd_autorizar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_creator(user):
        ...
        await update.message.reply_text(f"🔓 Bot autorizado correctamente para este grupo.\n🆔 Chat ID autorizado: `{current_chat_id}`")
        logger.info(f"✅ Bot autorizado por el creador en chat ID: {current_chat_id}")
    else:
        await update.message.reply_text(f"🚫 Solo {CREATOR_USERNAME} puede autorizar el uso del bot.")
        logger.warning(f"⛔ Usuario no autorizado intentó activar el bot: @{user.username}")
...
async def cmd_desautorizar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_creator(user):
        ...
        if authorized_chat_id and update.effective_chat.id != authorized_chat_id:
            await update.message.reply_text("🚫 Solo puedes desautorizar desde el chat autorizado.")
            return
        ...
        logger.warning(f"⛔ Usuario no autorizado intentó desactivar el bot: @{user.username}")
...
async def check_list_open(update):
    if not list_open:
        await update.message.reply_text("🚫 La lista está cerrada actualmente. Usa /abrir o /abrirlista para habilitarla.")
        return False
    return True
...
async def assign_zone(update, zone, context: ContextTypes.DEFAULT_TYPE):
    ...
    for current_zone, occupant in zones.items():
        if occupant == username:
            await update.message.reply_text(f"⚠️ Ya estás en la zona {current_zone[1]}⃣. Primero usa /exit para salir.")
            return
    
    # Verificar si el usuario está en la lista de espera
    if username in waiting_list:
        await update.message.reply_text("⚠️ Estás en la lista de espera. Usa /exit para salir de ella primero.")
        return
...
async def remove_zone(update, zone):
    ...
    if zones[zone] == username:
        ...
        await update.message.reply_text(f"⚠️ No estás en la zona {zone[1]}⃣ o no tienes permiso.")
    await update.message.reply_text(format_list())
...
async def cmd_espera(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        if not await is_admin(context, chat_id, user_id):
            await update.message.reply_text("🚫 Solo los administradores pueden añadir a otros usuarios a la lista de espera.")
            return
    ...
    for zone, occupant in zones.items():
        if occupant == username:
            await update.message.reply_text(f"⚠️ {username} ya está en la zona {zone[1]}⃣. Usa /exit para salir primero.")
            return
...
async def cmd_cambiar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) == 0:
        await update.message.reply_text("❌ Usa /cambiar @Usuario o /cambiar @Usuario1 @Usuario2")
        return
    ...
    if len(args) == 1:
        ...
        if not is_creator(user) and not await is_admin(context, update.effective_chat.id, user.id):
            await update.message.reply_text("🚫 Solo administradores pueden intercambiar a otros.")
            return
        username1, username2 = args
    else:
        await update.message.reply_text("❌ Formato incorrecto. Usa /cambiar @Usuario o /cambiar @Usuario1 @Usuario2")
        return
    ...
    if not pos1 and not pos2:
        await update.message.reply_text("❌ Ninguno de los usuarios está en zonas ni en lista de espera.")
        return
    ...
    if pos1 and pos2:
        if pos1[0] == "zone" and pos2[0] == "zone":
            ...
            waiting_list[pos1[1]], waiting_list[pos2[1]] = waiting_list[pos2[1]], waiting_list[pos1[1]]
    ...
    await update.message.reply_text(f"🔁 {username1} ha sido intercambiado con {username2}")
    await update.message.reply_text(format_list())
...
async def cmd_exit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if removed:
        if target_username == f"@{user_requesting.username}":
            ...
            await update.message.reply_text(f"✅ {target_username} fue removido correctamente.")
    else:
        await update.message.reply_text(f"⚠️ {target_username} no se encontraba en ninguna zona ni en la lista.")
...
async def cmd_tomarlibre(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for zone, occupant in zones.items():
        if occupant == username:
            await update.message.reply_text(f"⚠️ Ya estás en la zona {zone[1]}⃣. Primero usa /exit para salir.")
            return
    ...
    if libre_index is not None:
        ...
        await update.message.reply_text(f"✅ {username} tomó un lugar libre en la lista de espera")
...
async def cmd_abrir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_admin(context, chat_id, user_id):
        ...
        await update.message.reply_text("🔓 Lista abierta. ¡Ya puedes usar los comandos!")
        await update.message.reply_text(format_list())
        logger.info("Lista abierta")
    else:
        await update.message.reply_text("🚫 Solo los administradores pueden abrir la lista.")
...
async def cmd_cerrar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_admin(context, chat_id, user_id):
        ...
        await update.message.reply_text("🚫 Solo los administradores pueden cerrar la lista.")

async def cmd_reglas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await reject_private_messages(update): return
    if not await check_authorized(update): return
    await update.message.reply_text("""📜 Reglas del Sistema de Zonas:
...
async def cmd_comandos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_authorized(update): return
    await update.message.reply_text("""📌 Menú de Comandos:
...
async def cmd_chatid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_admin(context, chat_id, user_id):
        ...
        authorized_status = "✅ AUTORIZADO" if chat_id == authorized_chat_id else "❌ NO AUTORIZADO"
        ...
        logger.warning(f"⛔ Usuario no autorizado intentó obtener el ID del chat: @{update.effective_user.username}")
...
def main():
    ...
    app.add_handler(CommandHandler("z1", lambda update, context: assign_zone(update, "z1", context)))
    app.add_handler(CommandHandler("z2", lambda update, context: assign_zone(update, "z2", context)))
    app.add_handler(CommandHandler("z3", lambda update, context: assign_zone(update, "z3", context)))
    ...
    app.add_handler(CommandHandler("chatid", cmd_chatid))  # Comando para obtener el ID del chat
